...
def calculates_results_stats(results_dic):
    ...
    for key, value in results_dic.items():
        #label match
        if value[2] == 1: # dog_name == classifier_dog_name
            results_stats_dic["n_match"] += 1
        #pet image label is a dog and labels match -> count correct
        if sum(value[2:]) == 3:
            results_stats_dic["n_correct_breed"] += 1
        # count number of dogs image
        if value[3] == 1:
    ...
    #calculates run statistics (count & percentages) below that are calculated using the counters from above
    #total images
    results_stats_dic["n_images"] = len(results_dic)
    #calculates number of not-a-dog-images using images&dogs images count
    results_stats_dic['n_notdogs_img'] = results_stats_dic["n_images"] - results_stats_dic["n_dogs_img"]
    #calculates percent correct for matches
    try:
        results_stats_dic['pct_match'] = (results_stats_dic['n_match'] / results_stats_dic['n_images'])*100.0
    except:
        results_stats_dic['pct_match'] = 0
    #calculates percent correct dogs
    try:
    ...
    if results_stats_dic['n_notdogs_img'] > 0:
        ...
        results_stats_dic['pct_correct_ncotdogs'] = 0.0
